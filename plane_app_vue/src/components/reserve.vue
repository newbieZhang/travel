            ...
            <div class="personalDetail" v-for="(item,i) of list" :key="i">
                <table style="width:100%;">
                    ...
                    <tr>
                        ...
                        <td>
                            <input id="idCard" type="text" v-model="item.id">
                    ...
                    <tr>
                        <td>手机<span>*</span></td>
                        <td>
                            <input id="phone" type="text" v-model="item.pho">
...
export default {
    ...
    components:{traveldigest,myheader,myfooter},
    data(){
        return {
                ticket:1,
                originplace:"",
    ...
    methods:{
        ...
        next(){            
            for(var i = 0;i<this.passnum;i++){
                var uname=this.list[i].name;
                var id=this.list[i].id;
                var pho=this.list[i].pho;
            ...
            if(confirm("确定提交订单吗？")){
                for(var i = 0;i<this.passnum;i++){                
                    var uname=this.list[i].name;
                    var id=this.list[i].id;
                    var phone=this.list[i].pho;             
                    //数据库添加订单  
                    var gender = (this.list[i].sex=="男"?1:0);
                    var now = new Date().getTime();
                    var tkn = now+"001"+Math.floor(Math.random()*100);
                    //console.log("id="+id+"uname="+uname+"phone="+phone+"gender="+gender+"tkn="+tkn+"stime="+now+"chooseprice1"+this.chooseprice1+"fid1="+this.fid1)
                    this.axios.post(
                        "/pltk/add",
                    ...
                    if(this.ticket == 2){
                        tkn = now+"002"+Math.floor(Math.random()*100);
                        this.axios.post(
                            "/pltk/add",
                            `id=${id}&name=${uname}&phone=${phone}&gender=${gender}&tkn=${tkn}&stime=${now}&price=${this.chooseprice2}&fid=${this.fid2}`
                        ).then(result=>{
                            console.log(result);                    
                        })
                    }     
